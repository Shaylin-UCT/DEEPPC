...
│   │   InceptionTL.py
│   │   ParameterFIDScores
|   │   └───[csv folders generated by FIDTests.py]
|   |   |   ...  
...
|   VanillaGAN.py
|   WGANGP.py
│   WGANRuns [place .out and .err files here if running on cluster]
|   vanillaRuns [place .out and .err files here if running on cluster]
|   StyleRuns [place .out and .err files here if running on cluster]
└─── Generated Images
│   │   VanillaGAN
|   |   └─── Experiment1
|   |   |    Experiment2
...
|   |   |    ... 
|   |   StyleGAN2ADA
|   |   └─── [Generated folders (images + models) will be placed here]
│   │
│   └─── Images for Research
│   |   │   Dataset for MedFID
|   |   |   └─── valid (used as the test set)
|   |   |   |   └─── class 1
|   |   |   |   |   └─── x.png
|   |   |   |   |   |    ...
|   |   |   |   └─── ...
...
|   |   |   |   |   |    ...
|   |   |   |   └─── ...
|   |   |   Dataset for Style (Automatically generated - see README for details)
│   |   │   Dataset for GAN
│   |   │   └─── x.png
|   |   |   |    ...
```
